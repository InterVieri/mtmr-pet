...
class CustomButtonTouchBarItem: NSCustomTouchBarItem, NSGestureRecognizerDelegate {
    ...
    private func reinstallButton() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            ...
            let frameInWin = self.view.convert(self.view.bounds, to: nil)
            let superviewW = self.view.superview != nil ? Int(self.view.superview!.bounds.width) : -1
            let line = "id=\(self.identifier.rawValue) w=\(Int(f.width)) h=\(Int(f.height)) winX=\(Int(frameInWin.origin.x)) winY=\(Int(frameInWin.origin.y)) superview=\(superviewW)\n"
            let url = URL(fileURLWithPath: "/tmp/mtmr-widgets.log")
